...
const TourOverview: React.FC<Props> = ({ tour, onDateSelect }) => {
  ...
  const allDates = tour?.availableDates || [];
  ...
  const handleDateClick = (month: string, dateObject: any) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 mb-8">
      {/* Inject itinerary JSON-LD for SEO crawlers */}
      {itineraryJsonLd && (
        <Head>
          <script
                ...
                <div key={index} className="bg-white p-3 rounded-lg shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {monthData.dates.map((dateObj: any, i: number) => {
                      ...
                      const disabled = dateObj.enabled === false || past || (!disable20RuleForThisTour && within20);
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-5 sm:mb-6">
        {tour.attractions?.map((attraction: string, index: number) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
        {tour.gallery?.slice(0, 4).map((photo: string, index: number) => (
